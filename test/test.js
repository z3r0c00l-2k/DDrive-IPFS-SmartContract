...
contract("DDrive", ([deployer, uploader]) => {
  let dDrive;

  before(async () => {
    dDrive = await DDrive.deployed();
  });

  describe("deployment", async () => {
    it("deploys successfully", async () => {
      const address = await dDrive.address;
    ...
    it("has a name", async () => {
      const name = await dDrive.name();
      assert.equal(name, "DDrive");
    });
  });
  ...
  describe("file", async () => {
    ...
    before(async () => {
      result = await dDrive.uploadFile(
        fileHash,
        fileSize,
        fileType,
        fileName,
      ...
      fileCount = await dDrive.fileCount();
    });
    ...
    it("upload file", async () => {
      ...
      await dDrive.uploadFile(fileHash, fileSize, fileType, fileName, "", {
        from: uploader,
      }).should.be.rejected;
    });
    ...
    //check from Struct
    it("lists file", async () => {
      const file = await dDrive.files(fileCount);
      assert.equal(
